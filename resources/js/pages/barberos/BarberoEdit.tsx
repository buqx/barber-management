...
export default function BarberoEdit({ barbero }: Props) {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-4 flex flex-col gap-6">
      <Head title={`Editar Barbero: ${barbero.nombre}`} />
      <Card className="bg-neutral-900 border border-neutral-800 p-6">
        <div className="flex items-center gap-4 mb-4">
          <User className="text-[#D4AF37]" />
          <h2 className="text-xl font-bold text-neutral-100">Editar {barbero.nombre}</h2>
        </div>
        <form
          onSubmit={e => {
            e.preventDefault();
            form.put(`/barberos/${barbero.id}`);
          }}
        ...
        >
          ...
          <input
            type="text"
            value={form.data.especialidad}
            onChange={e => form.setData('especialidad', e.target.value)}
      ...
      <Link href={`/barberos/${barbero.id}`} className="text-[#D4AF37] font-medium hover:underline">Volver al detalle</Link>
    </div>
  );
}
